...
const UserController = {
  ...
  getByRoles: (req, res) => {
    const role = req.params.role; // Lấy vai trò từ tham số truy vấn
    User.getByRoles(role, (users) => { // Gọi phương thức trong model
      if (users === null) { // Nếu không tìm thấy dữ liệu
        return res.status(404).json({
          users: null,
          status: false,
          message: "Không tìm thấy dữ liệu",
        });
      } else { // Nếu tìm thấy dữ liệu
        return res.status(200).json({
          users: users,
          status: true,
          message: "Lấy dữ liệu thành công",
